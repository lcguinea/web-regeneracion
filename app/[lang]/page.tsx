...
export default function Home({ params }: { params: { lang: string } }) {
  ...
  return <main>
    ...
    <Section id="trabajo" className="band"><div className="split image-split"><div><div className="kicker">{p.trabajo.label}</div><h2>{p.trabajo.title}</h2><p className="prose">{p.trabajo.body}</p></div><div className="image-frame image-work"><Image src="/images/trabajo-estudio-geometria.png" alt={data.media.workAlt} fill sizes="(max-width: 380px) calc(100vw - 40px), (max-width: 800px) calc(100vw - 56px), 58vw"/></div></div></Section>
    <Section id="biblioteca"><div className="split"><div><div className="kicker">{p.biblioteca.label}</div><h2>{p.biblioteca.title}</h2></div><div><p className="prose">{p.biblioteca.body}</p><Link className="button" href={`/${lang}/biblioteca`}>Abrir la Biblioteca →</Link><div className="image-frame image-library"><Image src="/images/biblioteca-libros-estudio.png" alt={data.media.libraryAlt} fill sizes="(max-width: 380px) calc(100vw - 40px), (max-width: 800px) calc(100vw - 56px), 58vw"/></div></div></div></Section>
